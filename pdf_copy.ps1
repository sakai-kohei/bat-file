...
echo "[$date $title] バッチの実行を開始します。 " |  Add-Content $result -Encoding UTF8 –pass
do {
 $year = Read-Host "作成したレポートの年度を[yyyy]で入力してください。" |  Add-Content $result -Encoding UTF8 –pass
 if (($year -gt $kotosi) -Or ($year -lt 2009)) {
  echo "[$date $title] 入力された""$year""は無効です。もう一度入力してください。" |  Add-Content $result -Encoding UTF8 –pass
 }else{
  $i=0
 }
}while($i -eq 1)
 ...
 do {
  ...
  if(-not(($month -eq "01") -Or ($month -eq "02") -Or ($month -eq "03") -Or ($month -eq "04") -Or ($month -eq "05") -Or ($month -eq "06") -Or ($month -eq "07") -Or ($month -eq "08") -Or ($month -eq "09") -Or ($month -eq "10") -Or ($month -eq "11") -Or ($month -eq "12"))){
   echo "[$date $title] 入力された""$month""は無効です。もう一度入力してください。"  |  Add-Content $result -Encoding UTF8 –pass
  }else{
   $i=1
  }
}while($i -eq 0)
...
よろしければy、バッチの実行を停止したい場合はnを入力してください。" |  Add-Content $result -Encoding UTF8 –pass
if($ans -eq "y"){
 echo "[$date $title] 入力したフォルダを作成し、同期を開始します。" |  Add-Content $result -Encoding UTF8 –pass
}
else {
 echo "[$date $title] バッチの実行を中止します。 " |  Add-Content $result -Encoding UTF8 –pass
 exit
}
...
if($LASTEXITCODE -eq "1"){
 echo "[$date $title] バッチの実行により、同期が成功しました。" |  Add-Content $result -Encoding UTF8 –pass
 exit
}elseif($LASTEXITCODE -eq "0"){
 echo "[$date $title] 同期の必要がなかったため、バッチは実行されませんでした。" |  Add-Content $result -Encoding UTF8 –pass
 exit
}elseif($LASTEXITCODE -gt "1"){
 echo "[$date $title] エラーが発生したため、バッチの実行が失敗しました。" |  Add-Content $result -Encoding UTF8 –pass
}